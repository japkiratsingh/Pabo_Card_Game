...
public:
  ...
  int getTotalCards() { return cards.size(); }
};
...
public:
  ...
  bool operator==(const Player &other) const {
    return playerIndex == other.playerIndex; // Assuming playerIndex is a unique
...
public:
  // Constructor to initialize the game with a specified number of players and
  // decks
  ...
  void playGame() {
    cout << "We have " << players.size()
         << " players with us joining today. Hope you have a great "
            "experience\n\n";
    ...
    cout << "Game has ended. Player " << remainingPlayer.getPlayerIndex()
         << " is the loser!\n";
  }
...
private:
  Card UserTurn(Suit currentSuit, Player &starter) {
    ...
    if (starter == *User) {
      ...
      do {
        ...
        if (!isCardAvailableWithUser(*User, userPlayingCard)) {
          cout << "Come on man you don't have that card\n";
        } else if ((*User).hasCardOfSuit(currentSuit) &&
                   currentSuit != userPlayingCard.suit) {
          cout << "Hey yo. No cheating dude!! I Know, You Know, We Know\n";
        }
      } while (!isCardAvailableWithUser(*User, userPlayingCard) ||
               ((*User).hasCardOfSuit(currentSuit) &&
                currentSuit != userPlayingCard.suit));
    }

    // cout << "\nYou have chosen: Suit " << userPlayingCard.suitToString() <<
    // ", Rank " << userPlayingCard.rankToString() << "\n";
  ...
  Card selectUserCard(Player &player) {
    ...
    while (!(cin >> userSuit) || (userSuit < 1) || (userSuit > 4) ||
           (cin.peek() != '\n')) {
    ...
    cout << "\nYou have chosen: Suit " << userCard.suitToString() << ", Rank "
         << userCard.rankToString() << "\n\n";
    return userCard;
  }
  ...
  bool isValidRank(const string &card) {
    if (card.size() == 1) {
      ...
      return (isdigit(firstChar) && (firstChar >= '2' && firstChar <= '9')) ||
             ((firstChar) == 'A' || (firstChar) == 'J' || (firstChar) == 'Q' ||
              (firstChar) == 'K');
  ...
  void initializeGame() {
    ...
    for (Player &player : players) {
      for (int i = 0; i < cardsPerPlayer;
           ++i) { // Adjust the number of initial cards per player as needed
    ...
    typeWriterEffect("So who's gonna start now. Who has the Ace of "
                     "spades..................... ");
    usleep(2000*1000);
    // Find the player with the Ace of Spades and make them the starter
    auto starterIterator =
        find_if(players.begin(), players.end(), [](const Player &player) {
          return player.hasCardOfSuit(SPADES) &&
                 find_if(player.getHand().begin(), player.getHand().end(),
                         [](const Card &card) {
                           return card.rank == ACE && card.suit == SPADES;
                         }) != player.getHand().end();
        });

    if (starterIterator != players.end()) {
      starter =
          *starterIterator; // Dereference the iterator to get the Player object
      cout << "Player " << distance(players.begin(), starterIterator) + 1
           << " has the Ace of Spades and starts the game.\n";
    } else {
    ...
    do {
      ...
      if (!cardsOfCurrentSuit.empty()) {
        ...
        if (currentPlayer == *User) {
          ...
          auto maxCardIterator = max_element(
              cardsOfCurrentSuit.begin(), cardsOfCurrentSuit.end(),
        ...
        cout << "Player "
             << players[distance(players.begin(), currentPlayerIterator)]
                    .getPlayerIndex()
             << " played: Suit " << playedCard.suitToString() << ", Rank "
             << playedCard.rankToString() << "\n";
      } else {
        ...
        if (currentPlayer == *User) {
          ...
          auto maxCardIterator = max_element(
              currentPlayer.getHand().begin(), currentPlayer.getHand().end(),
              [](const Card &a, const Card &b) { return a.rank < b.rank; });
      ...
      if (currentPlayerIterator == players.end()) {
        currentPlayerIterator =
            players.begin(); // Wrap around to the first player
      }
    } while (currentPlayerIterator !=
             starterIterator); // Continue until we reach the starter again
  ...
  void playRound() {
    ...
    do {
      ...
      if (!cardsOfCurrentSuit.empty()) {
        ...
        if (currentPlayer == *User) {
          if (starter == *User) {
            playedCard =
                UserTurn(static_cast<Suit>(static_cast<int>(4)), starter);
    ...
    auto maxPlayedCardIterator =
        max_element(playedCards.begin(), playedCards.end(),
    ...
    Player &newStarter =
        players[(distance(playedCards.begin(), maxPlayedCardIterator) +
                 distance(players.begin(), starterIterator)) %
                players.size()];
    ...
    currentSuit = static_cast<Suit>(
        (static_cast<int>(maxPlayedCardIterator->suit) + 1) % 4);
  ...
  void printPlayersCards() const {
    ...
    for (const Player &player : players) {
      cout << "Player "
           << players[distance(players.begin(),
                               find(players.begin(), players.end(), player))]
                  .getPlayerIndex()
           << "'s cards: ";
      // for (const Card& card : player.getHand()) {
...
int main() {
  ...
  } else if (numDecks > 2 or numPlayers > 12 or
             (52 * numDecks) / numPlayers < 6) {
